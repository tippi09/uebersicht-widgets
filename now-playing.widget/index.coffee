...
font-size: 10px
margin = 30px
// vertical and horizontal coordinates px
verticalPX = 650px
horizontalPX = 30px
...
if #{options.verticalPosition} == center
    ...
    #{options.verticalPosition} margin // alternatively use margin or verticalPX

if #{options.horizontalPosition} == center
    left 50%
    transform translateX(-50%)
else
    #{options.horizontalPosition} margin // alternatively use margin or horizontalPX
...
.container
    ...
    clear: both
    color #fff
    background rgba(#000, .0) // dunkelgrau: .5 statt .0
    padding 10px
